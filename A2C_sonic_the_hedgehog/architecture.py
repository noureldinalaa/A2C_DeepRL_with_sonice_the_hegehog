#here we impelement the architecture
...
#this function selects the probability distribution over action
# **reminder for myself** ---> actor is policy based method as consequence we train
#                              on stochastic policy (pi(a|s = p(a|s))) ,
#                              reminder in deep mind lecture the example with fixed right and left is deterministic
#                              and the other one is stochastic
...
class A2CPolicy (object):
    def __init__(self, sess, ob_space, action_space, nbatch, nsteps, reuse = False):
        ...
        #this will use to inizialize our kernels
        gain = np.sqrt(2)
        ...
        """
        Architecture:
        illustrarion example to make it clear to myself:
        
        *  *  *       **********           ********       ------> Actor (pi(s) a set of action probabilities)
        *  I  *       *   CNN  *           *  FC  *    ---   
        *  I  *  -->  *   CNN  * --->      *  FC  * ---
        *  I  *       *   CNN  *           *  FC  *    ---
        *  *  *       **********           ********       -------> Critic (V(s) how good a certain state is to be in a certain action )
        """
        ...
        with tf.variable_scope("model",reuse=reuse):
            ...
            # Calculate the v(s)
            vf = fc_layer(fc_common, 1 , activations_fn=None)[:, 0]
